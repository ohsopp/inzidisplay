# MC Protocol(3E) & MQTT Monitor

MC Protocol(3E) 데이터와 MQTT(IOLink) 센서 데이터를 실시간으로 모니터링하는 웹 애플리케이션입니다.
...
2. 좌측 탭에서 **MC Protocol** 또는 **센서 대시보드(MQTT)** 선택
3. MC 화면에서 PLC IP/포트를 입력하고 **폴링 시작** 클릭
4. MQTT 센서는 수신 시 대시보드에 자동 반영
